...
export async function GET() {
	...
	try {
		...
		const [
			...
			prisma.supportConversation.count({ where: { status: { in: ['WAITING_AGENT', 'AGENT'] } } }),
		...
		const pendingDeposits = pendingWalletRows.find((row) => row.type === 'DEPOSIT')?._count || 0;
		const pendingWithdrawals = pendingWalletRows.find((row) => row.type === 'WITHDRAW')?._count || 0;
